...
- 管理端静态 JS/CSS 使用内容哈希 URL 并要求重新验证，避免 Cloudflare 或浏览器继续执行旧同步协议脚本。
- Alias checkbox 可保留多个选中状态，批量请求携带全部选中 ID，且不超过配置上限。
- 终态为 `needs_reconcile` 时仍可通过受 CSRF 保护的 POST results 获取已成功项，并输出“邮箱账号；解码网站；接码密钥”。
- 任务摘要分别计算成功、明确失败、远端结果不确定、尚未开始和处理中，不再把 `unknown` 误称为明确失败。
- HME 普通 API 请求吸收响应 Cookie 轮换；仅当数据库 Session 仍等于请求起点时持久化，防止旧客户端覆盖人工捕获或后台刷新后的新 Session。
- Apple 写操作结果不确定时保持停止且不自动重放；后续只读对账必须在能够精确确认 candidate 的前提下单独实施。
...
- [x] 本事故计划已提交为 `d7ffa3b`；两次初始推送均因 GitHub `HTTP2 framing layer`
  失败，诊断未因此停顿，改在最终记录完成后合并推送。
- [x] 只读核对生产版本、NTP、容器健康、IMAP 配置形状及 10:25-10:37 查询审计聚合。

#### 节点 U：目标邮件与控制链复现

- [x] 用一次只读 IMAP 登录扫描 10:25-10:37，不把邮件标为已读，不输出邮件内容或身份。
- [x] 将时段邮件收件人哈希与本地 Alias 关联，确认目标邮件、Alias 状态、key 和 sender filter。
- [x] 对目标邮件复现数字候选、语境、时间窗和当前提取结果，并与查询审计逐次对齐。

#### 节点 V：结论与收口

- [x] 给出唯一根因并确认由 `46c8bbb` 引入：HTML 可见文本未标准化空白，导致语境距离
  被排版字符放大后误拒验证码。
- [x] 更新本节实施记录和必要的运维记录，提交并推送 GitHub `main`，清理诊断临时件。
- [x] 未改生产代码、配置或镜像；是否实施修复等待用户明确授权。
...
- 2026-07-31：节点 T 完成。生产部署标记为
  `46c8bbb1515936119314cf899eaca6ad0b016255`，app 容器
  `783385aad7d3dc0c62fa1694b57c222d71c98fedb068677369cc251cb1d05767`，镜像为
  `sha256:eb34a40827f3a046f430a4e150b7ef43be165135da41fc68cccd3ab50b14daa3`。
  NTP 正常，app `healthy / restart=0 / OOM=false`，SQLite `quick_check=ok`，内外
  `/healthz` 均为 200，事故时段错误日志为 0。IMAP 只配置 INBOX，Junk 为空；读取路径为
  `BODY.PEEK[]`。
- 2026-07-31：节点 U 完成。10:30-10:35 同一 Alias 共 33 次公开查询，审计结果全部为
  `no_code`；INBOX 中存在 10:30、10:31、10:33 三封精确投递邮件。目标 Alias 活动、有
  key、sender filter 为空，且大量查询位于各邮件到达后的 300 秒有效窗内。三封邮件均为
  HTML-only，均且仅含一个独立六位数字；旧提取器可全部提取，`46c8bbb` 当前提取器全部
  拒绝。取证前后邮箱 Seen 数均为 4，确认只读扫描未改变邮件状态。
- 2026-07-31：节点 V 得到唯一根因。当前 `_message_body()` 去除 HTML 标签后未压缩标签
  间缩进和换行，语境评分把这些不可见排版空白计入字符距离；三封真实邮件中已识别验证码
  语境到六位数字的原始距离均为 167，超过门限 80。仅对可见文本执行
  `re.sub(r"\s+", " ", text)` 后，距离降为 4/5，现有语境词和门限即可接受。因此本次
  不是邮件延迟、QQ/Junk、Alias、key、sender filter、五分钟窗口、时钟或基础设施故障，
  也不需要扩充中文词表；直接原因是 `46c8bbb` 引入语境距离时遗漏 HTML 空白标准化。
- 2026-07-31：建议修复边界为：在验证码提取前标准化可见文本连续空白，并用本次
  HTML-only 布局形态补正向回归，同时保留无语境六位数字的反向用例。当前只完成只读取证，
  未修改代码、配置、镜像、数据库、邮件 Seen 状态或 Apple/HME 远端状态。
- 2026-07-31：节点 V 收口完成。计划提交 `d7ffa3b` 与诊断记录提交 `9122000` 已推送
  GitHub `main`；本地无诊断临时件，生产部署标记保持 `46c8bbb`。

## 21. HTML 验证码语境距离回归修复与生产部署计划（2026-07-31）

### 21.1 目标与性能指标

修复 `46c8bbb` 引入的 HTML-only 验证码漏检：邮件可见文本中的验证码语境与独立六位
数字相邻时，不得因 HTML 标签之间的缩进、换行和连续空白被错误判定为超过 80 字符；无
验证码语境的账号、订单号等六位数字仍必须拒绝。用户已明确要求修正，且此前明确要求直接
部署服务器，因此本节覆盖代码、测试、GitHub `main` 和 `icloud.yunbay.xyz` 的低停机部署。

验收指标：

- 新增一个不含真实服务、邮箱或验证码数据的 HTML-only 回归样本；该样本在修复前稳定失败，
  修复后稳定通过。
- 仅在提取前标准化可见文本连续空白，不扩充语境词表、不增大 80 字符门限、不恢复“任意
  六位数字即验证码”，也不改变 Alias、sender filter、300 秒窗口或 IMAP 文件夹逻辑。
- 现有“无语境六位数字拒绝”“最近语境候选优先”“中英文/HTML 语境接受”、公开查询与
  管理员批量扫描测试全部通过；Ruff、格式、Python 编译、Compose、diff 和秘密扫描通过。
- 候选镜像在隔离数据卷中 healthy；用 10:30 事故的同三封历史邮件做只读内存复验时，旧
  生产代码均拒绝、候选代码均接受，只输出计数/布尔量，取证前后 Seen 数不变。
- 构建和候选阶段旧 app 持续服务；正式切换只替换 app，60 秒内恢复 healthy 和内外 200，
  否则独立 watchdog 自动恢复部署前镜像，不等待人工操作。
...
| 控制元素 | 本次修复对象 |
| --- | --- |
| 目标 | 五分钟内稳定返回语境明确的 HTML 六位验证码，同时维持低误码率 |
| 被控对象 | 邮件可见文本、语境距离评分、app 镜像与生产 app 容器 |
| 测量 | 回归测试、语境距离、候选提取布尔结果、health、SQLite/容器状态 |
| 控制器 | 连续空白标准化、现有 80 字符门限、隔离候选、60 秒 watchdog |
| 执行 | 返回最新合格验证码；部署时只 force-recreate app；失败时恢复旧镜像 |
| 扰动 | HTML 缩进/换行、multipart 排版、邮件服务模板变化、构建/启动时延、网络波动 |
| 稳定性 | 先失败样本再修复；不放宽安全边界；先候选后切换；旧镜像和数据备份独立保留 |

最小充分闭环：

```text
脱敏 HTML 事故形态在旧代码稳定失败
  -> 连续空白标准化后语境距离进入既有 80 字符门限
  -> 正向样本通过且无语境反向样本仍拒绝
  -> 全量静态/测试门禁通过
  -> 生产基线、在线备份和旧镜像回滚点建立
  -> 隔离候选 healthy 且同三封历史邮件布尔复验 3/3 接受
  -> 独立 watchdog 就绪后只替换 app
  -> 60 秒内健康、数据、源码、部署标记和无关容器全部收敛
  -> 固定 release、记录、清理并推送 GitHub main
```

### 21.3 GitHub 经验与最小修复设计

- 已复核 `Redmig110/ic-veilmail` 的
  `2f94a551345407dbb937b519076d83006a868483`：验证码语境控制需要同时以正向真实形态和
  无语境反向形态校准，不能只追求降低误码率。本次沿用这一经验，用一正一反闭环约束修复。
- 事故取证已经排除词表缺失：三封邮件中的现有语境在压缩连续空白后距验证码仅 4/5；因此
  不增加语境词、不调整门限，只消除 HTML 排版空白对字符距离测量的干扰。
- 最小实现是在 `_extract_message_code()` 评分前对 `subject + body` 的连续空白执行一次
  `re.sub(r"\s+", " ", ...)`。不新增 helper、配置项或解析依赖，不改 `_message_body()` 的
  MIME/附件边界；空白不承载验证码语义，因此标准化同时适用于 plain text 和 HTML。
- 回归样本使用通用服务文案和多层 HTML 缩进构造大于 80 的原始排版距离，不复制事故邮件
  的主题、正文、发件人、Alias 或验证码；反向样本继续证明普通六位数字不会被返回。

### 21.4 实施节点与循环状态

#### 节点 W：计划、基线与失败样本

- [x] 确认事故根因、最小修复位置、现有测试边界和生产 app-only 部署模式；工作树在计划
  编辑前干净，本地/GitHub `main` 均为 `0809a04`。
- [x] 提交并推送本节完整计划到 GitHub `main`，再开始修改生产代码。
- [x] 增加脱敏 HTML-only 排版回归测试并证明旧代码失败；记录失败断言，不修改门限或词表。

停止条件：样本不能稳定复现、失败原因不是语境距离，或必须放宽六位数字安全边界才能通过。

#### 节点 X：最小实现与本地门禁

- [x] 在 `_extract_message_code()` 评分前标准化连续空白，不修改其他 IMAP 或 Web 路径。
- [x] 运行定向正/反回归，再运行全量测试、Ruff、格式、Python 编译、两套 Compose 配置、
  `git diff --check` 和秘密扫描。
- [x] 审查变更只包含计划、单点实现和对应测试；提交功能代码并推送 GitHub `main`。

停止条件：任何现有反向测试回退、全量门禁失败、变更扩大到无关功能，或仓库出现秘密。
...
- [x] 精确上传功能提交归档，在旧 app 在线时构建唯一候选；使用独立卷验证启动、Host-aware
  health、SQLite、旧加密配置和秘密边界。
- [x] 候选容器内只读复验 10:30 同三封历史邮件：只输出目标数、独立六位数计数、旧/新提取
  成功计数和 Seen 前后计数，不输出身份、正文、主题、发件人或验证码。

停止条件：主机身份不匹配、基线异常、备份/清单失败、磁盘不足、候选不健康、数据/秘密
边界失败，或真实邮件复验不是旧版 `0/3`、候选 `3/3`。停止时保持旧 app 在线。

#### 节点 Z：有界切换、生产验收与收口

- [x] 切换前启动独立 60 秒 watchdog；失败路径只恢复 rollback app 镜像并 force-recreate，
  成功或失败均不依赖当前 SSH 会话。
- [x] 只替换 app，确认 60 秒内 app healthy、内部 Host-aware 和公网 `/healthz` 为 200；成功
  后写信号停止 watchdog，失败则确认已自动回滚并停止发布。
- [x] 复验公网入口、日志、SQLite/数据指纹、运行镜像/源码/部署标记以及无关三容器状态；
...
- 本次没有 schema 迁移；普通启动、行为或健康回归只回滚 app 镜像，不恢复 SQLite。只有
  `quick_check` 或数据指纹异常时才保留故障现场并使用上线前在线备份。
- 连续空白标准化可能缩短纯文本中人为插入大量换行的语境距离；空白不携带语义，且代码仍
  必须满足已知语境、独立六位数字和 80 个可见字符门限，因此不视为放宽到无语境数字。
- 历史事故邮件已超过公开 300 秒窗口，不能通过网站再次领取；生产行为验收在内存中直接对
  同三封邮件运行提取器，只证明解析器修复，不修改业务时间窗口或邮件状态。
- SSH 中断不能让发布悬停：正式切换前必须验证 watchdog 独立运行，60 秒内自动选择新镜像
  健康或旧镜像回滚两种稳定状态之一。

### 21.6 本次实施记录

- 2026-07-31：用户明确要求修正。已确认最小实现为提取评分前连续空白标准化；本节建立时
  本地与 GitHub `main` 均为 `0809a04`，生产仍运行 `46c8bbb`，尚未修改代码或服务器。
- 2026-07-31：节点 W 的完整计划提交 `9a81c49` 已推送 GitHub `main`；开始增加脱敏失败
  样本，生产服务器尚未连接或修改。
- 2026-07-31：节点 W 失败闭环成立。脱敏 HTML 表格样本只在语境与数字之间加入 205 个
  排版空白字符，旧代码返回 `None`，定向测试在结果属性断言处失败；随后进入节点 X，仅在
  `_extract_message_code()` 评分前增加一次连续空白标准化，词表、门限和其他路径未改。
- 2026-07-31：节点 X 本地门禁通过。新增样本与无语境拒绝、最近候选、中英文 HTML 共
  6 个定向断言通过；全量 `146 passed`，仅有既有 TestClient/httpx 弃用警告。Ruff、格式、
  Python 编译、使用 `.env.example` 的本地/服务器 Compose、diff 和秘密扫描均通过。格式器
  仅在两个已修改文件中机械收敛 5 处旧换行，无行为变化；当前待提交功能代码。
- 2026-07-31：节点 X 完成。功能提交 `67968b7` 已推送 GitHub `main`；代码审查确认唯一
  行为变更是提取前连续空白标准化，新增一个事故形态测试，未修改安全门限或上游流程。
- 2026-07-31：节点 Y 基线与回滚点通过。生产仍为 `46c8bbb` 和镜像
  `sha256:eb34a40827f3a046f430a4e150b7ef43be165135da41fc68cccd3ab50b14daa3`；NTP、
  `.env=0600`、44GB 可用磁盘、四容器 `healthy / restart=0 / OOM=false`、内外 200、
  SQLite `ok` 和严重日志 0。数据基线为 159 个 Alias（148 活动）、41 个 key hash、37 个
  key 密文、299 条审计和 2 条设置，Alias/设置/审计前缀摘要已保存。
- 2026-07-31：节点 Y 备份与候选通过。审计目录为
  `/opt/new-api/icloud-code-gateway/backups/html-whitespace-20260731T042125Z-67968b7`，
  在线 SQLite 备份为 `ok`；旧镜像固定为
  `rollback-pre-67968b7-20260731T042125Z`。目标 Git 归档本地/服务器 SHA-256 均为
  `37534bc0be368492b0eb6a24f2d263eeecce93995424541882beff26ef7355ba`；候选镜像为
  `sha256:bdbc7846d2e89b07db0c48acaf7e5bd5cfd4f83475cfba29ccd882e23c4f341a`，revision
  为完整 `67968b7`。独立候选 healthy、内部 200、SQLite/数据计数和旧 IMAP 配置正常，
  合成正向接受、反向拒绝、日志秘密命中 0。
- 2026-07-31：节点 Y 真实邮件闭环通过。旧生产与候选均只读定位到同一 Alias 的 33 次
  `no_code` 审计和三封目标邮件；Alias 活动、有 key、sender filter 为空，每封均只有一个
  独立六位数字。旧版提取 `0/3`，候选提取 `3/3`；两次扫描 Seen 总数前后均完全不变，
  未输出身份、主题、正文、发件人或验证码。当前进入节点 Z。
- 2026-07-31：节点 Z 切换通过。独立 watchdog 的本地/服务器脚本哈希一致并在切换前启动；
  只 force-recreate app，新容器
  `e28b19bd6c8120235735d6cb31dfc3ad1989e594f9f79b814627cbb3f1f812f1` 使用候选镜像，
  `14.387s` 内完成 healthy、内部/公网 200、源码和完整部署标记闭环，watchdog=`accepted`，
  未触发回滚。切换期本机采样脚本因 zsh 保留变量名在请求前退出，因此以控制器闭环时间
  作为公网不可用保守上界，不把无效样本包装成停机证据。
- 2026-07-31：节点 Z 生产验收通过。运行 revision、完整 marker 和目标 Git 归档一致，
  77 个归档文件差异为 0；SQLite 前后均 `ok`，159/148 个 Alias、41/37 个 key、299 条
  审计、2 条设置及三类摘要逐字一致。公网 health/首页/登录为 200，未登录 noVNC 为 303，
  服务器后续 20/20 health 为 200；app 严重日志、Caddy 502 和运行日志秘密命中均为 0。
  browser、cn-proxy、Caddy 的 ID/restart/OOM 未变。当前只待固定标签、清理和三处记录。
- 2026-07-31：节点 Z 服务器清理完成。新镜像固定为 `latest/prod/release-67968b7`，旧镜像
  `sha256:eb34a40827f3a046f430a4e150b7ef43be165135da41fc68cccd3ab50b14daa3` 只保留
  `rollback-pre-67968b7-20260731T042125Z`。候选容器/卷/网络/标签、旧 release 标签、含
  临时环境副本的发布目录和部署锁均已删除；根卷仍可用 44GB。审计目录保留 22 个权限
