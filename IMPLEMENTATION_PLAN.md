...
- [ ] 提交并推送本节完整计划到 GitHub `main`，再开始修改生产代码。
- [ ] 增加脱敏 HTML-only 排版回归测试并证明旧代码失败；记录失败断言，不修改门限或词表。

停止条件：样本不能稳定复现、失败原因不是语境距离，或必须放宽六位数字安全边界才能通过。

#### 节点 X：最小实现与本地门禁

- [ ] 在 `_extract_message_code()` 评分前标准化连续空白，不修改其他 IMAP 或 Web 路径。
- [ ] 运行定向正/反回归，再运行全量测试、Ruff、格式、Python 编译、两套 Compose 配置、
