...
- 2026-08-27：按用户要求把 iCloud 邮件项目从云贝生产服务器 `13.140.180.223` 整体迁到服务器 2 `169.58.222.148`。功能提交 `b6ee2919edfda018b3f9482f068e6b64107cdcce` 把管理员入口收敛为同一站点的“邮箱管理 / 邮件收件箱”两个工作区，管理员 Cookie 默认 30 天、Worker 操作员 Cookie 独立 24 小时；HME Session 继续用主密钥加密写入 SQLite，浏览器 profile 使用独立持久卷。本地 control 新增 `POST /admin/api/hme-session/import` 的 Bearer control-token 上传兜底，捕获成功先保存在本机，再经 HTTPS 自动上传服务器；上传失败不要求重新 Apple 登录。移动端头部修复提交 `19897ec` 已发布，390×844 实图中站点名与两个工作区均保持单行、无横向溢出。
