...
- 创建任务会加密保存 generate 候选。reserve 断线后必须先读取并校验完整 HME 快照：候选精确存在时补齐本地记录；网络丢响应且可信快照明确不存在时才重试同一候选；Apple 明确限额/拒绝且快照确认未创建时丢弃可能过期的候选，冷却或退避后重新 generate。列表/Session 暂不可用时保持 queued；不得跳过只读对账盲重放。
- Apple `-41012` 是账户隐藏邮箱总容量已满，不是临时限流。批任务必须在第一个 generate 处停止并保留剩余 queued，管理页提示永久删除不再使用的 Alias；deactivate 不释放容量，禁止定时盲重试或把后续全部记为失败。
...
生产服务器上的同机既有项目代理只绑定其网络命名空间回环地址，不能让 iCloud app/browser 直接加入该命名空间，否则双方重启边界会耦合。生产采用以下方式：
...
- 同机既有项目现有 proxy/worker 容器、配置文件和数据目录不修改、不重启。
...
- 2026-08-29：最终 app-only watchdog 28 秒接受镜像 `sha256:76161f7d8fc3dab6a04bf63fb44aed6f3d8640a5f963adcef5752651159b9180`，marker/`latest`/`prod`/`release-7645ac8` 均为 `7645ac8e77df6098e3a423f81125f57cf3106f1e`；回滚标签 `rollback-pre-batch-resume-final-20260828T203630Z` 指向第一阶段镜像 `877ff9cc5418…`。冷却到期实测旧候选被清除，fresh candidate 再次遇到 `-41015` 后也立即丢弃，目标任务保持 `queued / failed=0 / candidate_present=false` 并等待下一轮自动续跑，没有回到终态。
- 2026-08-29：最终 SQLite `quick_check=ok`，695 Alias / 677 active / 417 keyed / 5776 audit / 198 job / 1023 item / 2 setting；app `5f61348c5df3…` healthy/restart=0/OOM=false，browser/cn-proxy/Caddy ID 未变。稳定期管理入口 20/20 为 200，app 严重日志与最近 Caddy 5xx/error 为 0。审计目录 `/opt/new-api/icloud-code-gateway/backups/batch-resume-final-20260828T203630Z-7645ac8` 的清单 SHA-256 为 `8d5f403dc212992bc5d6f678bceebca9659fcd8646edd4088703a846e6ca56a3`；候选容器、卷、标签、staging 与未切换预检目录均已清零。
- 2026-08-27：按用户要求把 iCloud 邮件项目从生产服务器 `<server-1-ip>` 整体迁到服务器 2 `<server-2-ip>`。功能提交 `b6ee2919edfda018b3f9482f068e6b64107cdcce` 把管理员入口收敛为同一站点的“邮箱管理 / 邮件收件箱”两个工作区，管理员 Cookie 默认 30 天、Worker 操作员 Cookie 独立 24 小时；HME Session 继续用主密钥加密写入 SQLite，浏览器 profile 使用独立持久卷。本地 control 新增 `POST /admin/api/hme-session/import` 的 Bearer control-token 上传兜底，捕获成功先保存在本机，再经 HTTPS 自动上传服务器；上传失败不要求重新 Apple 登录。移动端头部修复提交 `19897ec` 已发布，390×844 实图中站点名与两个工作区均保持单行、无横向溢出。
- 2026-08-27：服务器 2 使用 Ubuntu 24.04 官方 Docker `29.1.3` / Compose `2.40.3`，生产目录 `/opt/new-api/icloud-code-gateway/app`，独立 Caddy 监听 80/443，业务 browser 原始 noVNC 只绑定 `127.0.0.1:16081`，避免与服务器桌面 noVNC 的 `127.0.0.1:6080` 冲突。app/browser 镜像分别为 `sha256:b427394e3095586597a2d5638e5ff4e4b9e7ef1a07a7c0abc06bdedafe58356a` 与 `sha256:efd85de5fc60f693345fdd1eb1239ec945ffe5dee1b800e2ed975b0fd06271a8`，均固定 `latest/prod/release-b6ee291`；app、browser、cn-proxy、Caddy 最终为 running，前三者 healthy，四者 `restart=0 / OOM=false`。部署标记保持功能提交 `b6ee2919edfda018b3f9482f068e6b64107cdcce`，纯 Worker/CSS 和运维文档提交不覆盖该标记。
- 2026-08-27：一致性快照 `/opt/new-api/icloud-code-gateway/backups/migration-server2-20260826T213718Z` 包含 SQLite、browser profile、cn-proxy 数据和 iCloud 专用证书，`SHA256SUMS` 摘要 `3f71edebb9008751f82d38a661538428542116b7926bb7b96f0ebed0b3853219`；最终停写数据库位于 `backups/final-cutover-20260826T223044Z/gateway.sqlite3`，SHA-256 `5ab28df81b8e975c8395b9273365f7b3a5e6bfa40630fa6ecdb8261f751bdf3b`，其清单摘要 `4bb77d8b94294cf0bd7ab14ec3cd896d920c71b0c4395b9b05a8c2f7cf5dba09`。最终 SQLite `quick_check=ok`，636 Alias、15122 audit、191 batch job、943 item、2 setting、active job=0；迁移前尚未执行 7 天审计清理的 18421 条版本仍保存在第一份一致性快照。
- 2026-08-27：切流后使用迁移的加密 HME Session 在服务器 2 执行一次 Apple list 只读对账，HTTP 303 到 `sync_done`，管理页显示“连接正常”；没有 create/deactivate/reactivate/delete，Alias 仍为 636（618 active、386 keyed），active job=0。新的首选恢复点为 `backups/post-cutover-validation-20260826T231843Z/gateway.sqlite3`，SHA-256 `bec11c1744c36a643e6698e4e32dd445e292496e10e484072fb42654aaf554ca`，清单摘要 `1a74c55830f912d49d298716460865e7fc28d9c2a1a1269648b0143fb218846b`；SQLite `quick_check=ok`，15130 audit、191 job、943 item、2 setting。
- 2026-08-27：Cloudflare 已把已代理、自动 TTL 的 `icloud.yunbay.xyz A` 从 `<server-1-ip>` 改为 `<server-2-ip>`。Worker 最终版本 `63c3ac04-d76c-4b73-a346-3c494de31093`，用户/D1/Email Routing 数据未迁移也未中断。公开验收覆盖 Worker `/healthz`、`/readyz`、管理员登录、30 天 Cookie、两工作区、24 小时操作员 SSO、登录后 noVNC、CDP、Session 上传鉴权和新 CSS；首轮管理员入口 59/60 为 200、唯一失败没有服务端 HTTP 状态，Caddy 5xx/错误为 0，复核 30/30 为 200。第一次最终备份编排触发保护性回退，旧 app 自动恢复；第二次从源端停写到目标 ready 与 DNS 首个新 CSS 约 70 秒，略高于 60 秒目标，但 Worker/D1 的收信、用户取码与邮件 API 全程保持 200，VPS 管理 origin 才经历该切换窗口。
- 2026-08-27：旧生产服务器已从共享 Caddy 原位移除 iCloud 站点并 graceful reload，Caddy ID 不变、restart=0；iCloud 的 app/browser/cn-proxy 容器、3 个卷、专用网络、全部历史 app/browser 镜像标签和 `/opt/new-api/icloud-code-gateway` 项目目录均清为 0。根盘可用空间从 30GB 增至 39GB；该机共享 Caddy、New API、PostgreSQL、Redis、Sub2API 的容器 ID、restart=0、healthy 状态均未变化。旧机只保留 91MB 最小恢复包 `/opt/new-api/retired/icloud-code-gateway-server2-cutover-20260827`，清单摘要 `717407a6cb4168784a9acf4d422425f7490362593cf63f2d7592c590f00267ea`；普通回滚优先在服务器 2 使用 `release-b6ee291` 与上述备份，只有必须把 DNS 切回旧机时，才先从该恢复包重建旧 Caddy 站点、项目目录、卷和容器，验证健康后再改 DNS，不能只把 A 记录改回已退役的源站。
- 2026-08-26：按用户要求将 iCloud 管理与操作员邮件后台合并为一次登录、同域直接切换。提交 `8445ad8` 新增服务器端 `OperatorSsoClient`：只有已经通过 `icg_admin` 管理员 Session 的 `/admin/operator-session` 才能使用服务器 0600 环境中的 `ICLOUD_GATEWAY_OPERATOR_ACCESS_TOKEN`，向 Worker `/api/operator/session` 换取 `__Host-icg_mailbox` HttpOnly/Secure/SameSite=Strict Cookie；Operator Token 不进入 HTML、URL、JS、localStorage/sessionStorage 或日志。管理页“邮件后台”改走该 SSO 入口；`icloud.yunbay.xyz/admin/mail/` 的邮件会话缺失或过期时自动回到 SSO 换新后返回邮件列表，不显示 Token 登录表单；邮件页右上角改为“iCloud 管理”直接切回 `/admin`。`mailbox.yunbay.xyz/admin/` 仍保留独立 Token 登录，供应急维护。
- 2026-08-26：统一退出会同时删除 `icg_admin` 与 `__Host-icg_mailbox`，SSO 上游失败只回到安全提示，不返回 Token 或原始错误。全量 Python 测试、Ruff、Worker 26 项测试、Prettier、TypeScript、Compose config 与 Wrangler dry-run 均通过。生产 Worker 版本 `6ec0837b-954f-4246-8a75-81c4b2aa3f4f`；app 镜像 `sha256:2f9aa355d9bb7f7a86be3f1ead2f1bc36a5678043c4e2431405b774e1b664019`，回滚标签 `icloud-code-gateway-app:rollback-pre-unified-admin-20260826T113723Z`，browser/profile、Caddy 与 cn-proxy 未重启。环境备份位于 `/opt/new-api/icloud-code-gateway/backups/unified-admin-20260826T113225Z-8445ad8`，清单摘要 `499eefa795a9b5326894dd04cfd003d830e4da8a94b5538ee4d562afaac31916`。
- 2026-08-26：生产 HTTP 合同实测管理员登录 303、SSO 303 到 `/admin/mail/`、操作员 Cookie 合规、`GET /api/operator/session` 为 authenticated、邮件 API 200；退出响应同时清除两枚 Cookie。隔离真实 Chromium 从全新上下文只输入一次管理员密码，邮件页最终显示 `50+` 封并可切回管理页；定向删除邮件 Cookie 后自动 SSO 恢复 50 行，`operator-entry` 始终隐藏。app 切换期间后台 3 秒轮询出现 2 次短暂 502，约 4 秒后恢复，未影响 Worker 用户取码链；最终 app/browser/Caddy healthy、restart=0、OOM=false，app 严重日志 0。发布期间新批次又创建 5 个 Alias，SQLite 最终 627 条（609 active、377 keyed），D1 为 610 active/378 keyed，仍精确相差操作员 1 条。
...
- 附带：`scripts/sync-local-keys-to-edge.py` 与 `连接生产服务器.command` 的旧固定桌面路径已改为可配置路径（`ICLOUD_GATEWAY_CREDENTIALS_FILE` 或 `~/.icloud-code-gateway/`，带回退）。
...
- 控制面共享 token 与本地密码：`~/.icloud-code-gateway/icloud-control-plane.env`（0600）。
- 快捷入口：`<本地工具目录>/打开本地iCloud控制台.command`。
