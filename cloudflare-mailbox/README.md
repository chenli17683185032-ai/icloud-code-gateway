...
“加载全部”逐页显示加载进度，短暂网络故障会有限重试；手动刷新可重新建立分页位置，长时间切到后台后也可补齐期间的新邮件。操作员历史列表、HTML 和附件按 `retention_class=permanent` 判定长期保存，迁移前遗留的过期时间不会再隐藏长期邮件；普通用户验证码接口仍遵守原有过期时间。已按临时邮件保留策略清理的记录不在“全部历史”范围内。
